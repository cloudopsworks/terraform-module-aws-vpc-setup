...
resource "aws_secretsmanager_secret" "bastion_public_key" {
  count = var.bastion.create && var.vpc.secrets_manager.enabled ? 1 : 0
...
resource "aws_ssm_parameter" "tronador_accelerate_bastion_key" {
  count = var.bastion.create && var.vpc.secrets_manager.enabled && var.bastion.devops_accelerator ? 1 : 0
...
resource "aws_ssm_parameter" "tronador_accelerate_bastion_instance_user" {
  ...
  type  = "String"
  value = local.bastion_map[var.bastion.vendor].default_user
}
